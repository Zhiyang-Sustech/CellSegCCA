...
import CFunctionUtils as utils
...
def build_fpn_mask_graph(rois, feature_maps, image_meta,
                         pool_size, num_classes, vec_dim, train_bn=True):
    ...
    # 添加DCT
    # Two 1024 FC layers (implemented with Conv2D for consistency)
    x = KL.TimeDistributed(KL.Conv2D(1024, (14, 14), kernel_initializer='glorot_uniform'), name="mrcnn_mask_fc1")(x)
    x = KL.TimeDistributed(BatchNorm(), name='mrcnn_mask_fc_bn1')(x, training=train_bn)
    x = KL.Activation('relu')(x)

    x = KL.TimeDistributed(KL.Conv2D(1024, (1, 1), kernel_initializer='glorot_uniform'), name="mrcnn_mask_fc2")(x)
    x = KL.TimeDistributed(BatchNorm(), name='mrcnn_mask_fc_bn2')(x, training=train_bn)
    x = KL.Activation('relu')(x)
    # DCT mask output [batch, num_rois, 1, 1, 300] 编码为300维向量
    x = KL.TimeDistributed(KL.Dense(vec_dim, kernel_initializer='random_normal', name='mrcnn_mask_fc3'), name='mrcnn_mask_fc3')(x)
    return x
...
def mrcnn_mask_loss_graph(target_masks, target_class_ids, pred_masks):
    """Mask binary cross-entropy loss for the masks head.
        ...
        pred_masks: after DCT, [batch, proposals, 1, 1, 300].
        """
    # Only positive ROIs contribute to the loss. And only
    # the class specific mask of each ROI.
    target_class_ids = K.reshape(target_class_ids, (-1,))       #[N] N = batch * nun_rois
    mask_shape = tf.shape(input=target_masks)
    target_masks = K.reshape(target_masks, (-1, mask_shape[2], mask_shape[3]))          #[N, height, width]
    pred_masks = K.squeeze(K.squeeze(pred_masks, axis=2), axis=2)
    pred_shape = tf.shape(input=pred_masks)
    pred_masks = K.reshape(pred_masks, (-1, pred_shape[2]))         # [N, 300]
    ...
    # 用z字形的坐标表示向量, 并将gt_mask进行DCT编码
    dct_vector_coords = utils.get_dct_vector_coords()
    gt_masks = utils.encode(y_true, dct_vector_coords)  # [N, dct_v_dim]

    gt_masks = tf.cast(gt_masks, tf.float32)
    mask_loss = K.switch(tf.size(input=gt_masks) > 0, smooth_l1_loss(y_true=gt_masks, y_pred=y_pred), tf.constant(0.0))
...
class MaskRCNN(object):
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            # before DCT, Masks [batch, num_rois, MASK_POOL_SIZE, MASK_POOL_SIZE, NUM_CLASSES]
            # after DCT, Masks [batch, num_rois, 1, 1, 300]
    ...
    def set_trainable(self, layer_regex, keras_model=None, indent=0, verbose=1):
        ...
        if verbose > 0 and keras_model is None:
            ...
            log.print_info("Selecting layers to train", "info")
        ...
        for layer in layers:
            ...
            # if trainable and verbose > 0:
                ...
                # layers_log += "{}{:20}   ({})\n".format(" " * indent, layer.name, layer.__class__.__name__)
        log.print_info(layers_log, "info")
    ...
    def scheduler(self, epoch, lr):
        # if epoch == 20:
        #     print('epoch:',epoch + 1, 'current lr:', lr / 10)
        #     return lr / 10
        # elif epoch == 50:
        #     print('epoch:',epoch + 1, 'current lr:', lr / 3)
        #     return lr / 3
        # else:
        #     print('epoch:',epoch + 1, 'current lr:', lr)
        #     return lr
        return lr
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        # 20230315
        callbacks += [LearningRateScheduler(self.scheduler)]

        # Train
        log = CLogging()
        # log("\nStarting at epoch {}. LR={}\n".format(self.epoch, learning_rate))
        # log("Checkpoint Path: {}".format(self.checkpoint_path))
        log.print_info("Starting at epoch {}. LR={}".format(self.epoch, learning_rate), "info")
        log.print_info("Checkpoint Path: {}".format(self.checkpoint_path), "info")
    ...
    def unmold_detections(self, detections, mrcnn_mask, original_image_shape,
        ...
        mrcnn_mask: [N, height, width, num_classes], after DCT [N, 1, 1, 300]
        ...
        mrcnn_mask = np.squeeze(mrcnn_mask)
        masks = mrcnn_mask[:N, :]
        ...
        dct_vector_coords = utils.get_dct_vector_coords()
        masks = utils.decode(masks, dct_vector_coords, self.config.VEC_DIM, self.config.MASK_SHAPE[0])
