...
def unmold_mask(mask, bbox, image_shape):
    ...
    # img = Image.fromarray(np.array(mask) * 250)
    # img = img.convert('RGB')
    # img.save("dctmask.png")
...
def get_dct_vector_coords(r=128):
    """
...
def idct_2d(feature_map, norm='ortho'):
...
def idct_2d_np(feature_map, norm='ortho'):
    X1 = idct(feature_map, type=2, norm=norm)
    X1_t = np.transpose(X1, axes=(0, 2, 1))
    X2 = idct(X1_t, type=2, norm=norm)
    X2_t = np.transpose(X2, axes=(0, 2, 1))
    return X2_t

def encode(masks, dct_vector_coords, vec_dim=300, dim=None):
    """
    Encode the mask to vector of vec_dim or specific dimention.
    """
    if dim is None:
        dct_vector_coords = dct_vector_coords[:vec_dim]
    else:
        dct_vector_coords = dct_vector_coords[:dim]

    xs, ys = dct_vector_coords[:, 0], dct_vector_coords[:, 1]

    dct_all = dct_2d(masks, norm='ortho')
    dct_all = tf.transpose(a=dct_all, perm=[1, 2, 0])
    indices = tf.stack([xs, ys], axis=1)
    dct_vectors = tf.gather_nd(dct_all, indices)
    dct_vectors = tf.transpose(a=dct_vectors, perm=[1, 0])
    # dct_vectors = dct_all[:, xs, ys]  # reshape as vector
    return dct_vectors  # [N, D]

def decode(dct_vectors, dct_vector_coords, vec_dim, mask_size, dim=None ):
    """
    ...
    # dct_trans =  tf.zeros([N, mask_size, mask_size], dtype=dct_vectors.dtype)
    # dct_trans[:, xs, ys] = dct_vectors
    dct_trans = np.zeros([N, mask_size, mask_size], dtype=np.float32)
    xs, ys = dct_vector_coords[:, 0], dct_vector_coords[:, 1]
    for i in range(N):
        for j in range(vec_dim):
            dct_trans[i,xs[j],ys[j]] = dct_vectors[i, j]
    # dct_trans = tf.convert_to_tensor(dct_trans)
    # mask_rc = idct_2d(dct_trans, norm='ortho')  # [N, mask_size, mask_size]
    mask_rc_np = idct_2d_np(dct_trans, norm='ortho')
    return mask_rc_np
