...
def run_nuclei_type_stat(pred_dir, true_dir, type_uid_list=None, exhaustive=True):
    ...
    """
    file_list = glob.glob(true_dir + "*.mat")
    ...
    for file_idx, filename in enumerate(file_list[:]):
        ...
        pred_info = sio.loadmat(os.path.join(pred_dir, basename + "_fake.mat"))
        # dont squeeze, may be 1 instance exist
...
def run_nuclei_inst_stat(pred_dir, true_dir, print_img_stats=False, ext=".mat"):
    ...
    file_list = glob.glob("%s/*%s" % (true_dir, ext))
    ...
    for filename in file_list[:]:
        ...
        if not os.path.exists(pred_path):
            if not np.any(true):
                metrics[0].append(1.0)
                metrics[1].append(1.0)
                metrics[2].append(1.0)
                metrics[3].append(1.0)
                metrics[4].append(1.0)
                metrics[5].append(1.0)
            else:
                metrics[0].append(0)
                metrics[1].append(0)
                metrics[2].append(0)
        ...
        if not np.any(pred) and not np.any(true):
            ...
            metrics[0].append(get_dice_1(true, pred))   # dice
            metrics[1].append(get_fast_aji(true, pred)) #AJI
...
if __name__ == "__main__":
    # run_nuclei_type_stat('D:\hovernet-test\GAN2consep\consep_class_test\mat\\', 'D:\Data\CoNSeP\CoNSeP\Test\Labels\\')
    run_nuclei_inst_stat('D:\DVPszy\data\CC_doctor\\new_dataset_for_train\\test\cancer\cancer_test\mat\\', 'D:\DVPszy\data\CC_doctor\\new_dataset_for_train\\test\cancer\masks\\', print_img_stats=True)
